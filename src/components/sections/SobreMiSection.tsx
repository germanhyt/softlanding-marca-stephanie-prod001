...
type SobreMiSectionProps = {
  content: SiteContent;
};

export default function SobreMiSection({ content }: SobreMiSectionProps) {
  const { sobreMi, ui, whatsappMessage } = content;
        ...
        <div className="grid gap-12 md:grid-cols-[minmax(0,1.02fr)_minmax(0,1.02fr)] md:items-start md:gap-10 lg:gap-16 xl:gap-20">
          <motion.div className="order-2 leading-none md:order-1 md:self-end" {...slideFromLeft}>
            <img
              ...
              alt={sobreMi.portraitAlt}
              className="mx-auto block h-auto w-full max-w-sm object-contain object-bottom md:mx-0 md:max-w-none"
              loading="lazy"
            />
          </motion.div>

          <motion.div className="order-1 pb-16 md:order-2" {...slideFromRight}>
            <h2 className="text-3xl leading-[1.15] text-white md:text-4xl lg:text-[2.75rem] lg:leading-[1.12]">
              <span className="font-sans font-bold">{sobreMi.titleSans}</span>
              <span className="font-serif italic font-medium">{sobreMi.titleSerif}</span>
            </h2>
            ...
            >
              ...
              <p className="mt-1 font-serif text-sm italic text-white/80">{sobreMi.role}</p>
            </motion.div>
